...
<body class="bg-light">
    ...
    <div class="container mt-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="text-muted">{{ $university->uni_name }}</div>
        </div>

        <!-- Filter Card -->
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-body">
                <form method="GET" action="{{ route('admin_update_scholarships') }}">
                    <div class="form-row">
                        <!-- Department Filter -->
                        ...
                        <div class="col-md-4 mb-2">
                            ...
                            <select name="scholarship_status" class="form-control">
                                ...
                                <option value="Recipient" {{ request('scholarship_status') == 'Recipient' ? 'selected' : '' }}>Recipient</option>
                    ...
                    <div class="form-row mt-2">
                        <div class="col-md-12 text-right">
                            ...
                            <a href="{{ route('admin_update_scholarships') }}" class="btn btn-light px-4 ml-2">
                        ...
                        <tbody>
                            @forelse($scholarships as $scholarship)
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge {{ $scholarship->status == 'Recipient' ? 'badge-success' : 'badge-danger' }}">
                                            <i class="fas fa-{{ $scholarship->status == 'Recipient' ? 'check-circle' : 'times-circle' }} mr-1"></i>
                                            {{ $scholarship->status }}
                                        </span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <form method="POST" action="{{ route('admin_update_scholarship_status', $scholarship->scholarship_id) }}" class="d-inline">
                                            @csrf
                                            @method('PATCH')
                                            <div class="input-group">
                                                <select name="status" class="form-control form-control-sm">
                                                    <option value="Recipient" {{ $scholarship->status == 'Recipient' ? 'selected' : '' }}>Recipient</option>
